"""
Script para automatizar el ajuste de inventario A- 3.
"""
import time
import os
import logging
...
ajuste_inventario_ajuste3_info = {}
...
def ingresar_referencia_producto(driver, wait, capturas, textos, nombre_automatizacion, referencia="12957"):
    try:
        # Solo escribir la referencia
        buscar_referencia = wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@id='NombreProductoBusq']")))
        buscar_referencia.clear()
        buscar_referencia.send_keys(referencia)
        
        # Ocultar el menú de sugerencias de Typeahead
        driver.execute_script("""
            var menu = document.querySelector('.tt-menu');
            if (menu) {
        ...
        ajuste_inventario_ajuste3_info["referencia_producto"] = referencia
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def seleccionar_tipo_transaccion(driver, wait, capturas, textos, nombre_automatizacion, tipo_transaccion="A-"):
    try:
        ...
        ajuste_inventario_ajuste3_info["tipo_transaccion"] = tipo_transaccion
...
def seleccionar_causal_transaccion(driver, wait, capturas, textos, nombre_automatizacion, causal_index=3):
    try:
        ...
        if causal_index < len(opciones):
            ...
            ajuste_inventario_ajuste3_info["causal_transaccion"] = f"Índice {causal_index}"
            time.sleep(2)  # Esperar antes de continuar con la siguiente acción
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def ingresar_cantidad_fracciones(driver, wait, capturas, textos, nombre_automatizacion, fracciones="1"):
    try:
        ...
        ajuste_inventario_ajuste3_info["cantidad_fracciones"] = fracciones
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def capturar_alerta_final(driver, wait, capturas, textos, nombre_automatizacion, timeout=12):
    """Espera la alerta con id 'alertaPpal' y la captura usando su texto.

    Si no aparece en el timeout, toma una captura de fallback y registra la advertencia.
    """
    try:
        ...
        # Hacer scroll hacia arriba sobre la alerta para asegurar que quede visible en la captura
        try:
            driver.execute_script("arguments[0].scrollIntoView(true); window.scrollBy(0, -100);", alerta_elem)
        except Exception:
            # Si falla el scroll, continuar de todas formas
            pass
        time.sleep(0.5)
...
def inventario_ajuste3(driver, referencia="12957", tipo_transaccion="A-", causal_index=3, fracciones="1"):
    """
    Automatiza el proceso de ajuste de inventario 3 en el sistema, con logs y capturas.
    """
    wait = WebDriverWait(driver, 40)
    capturas = []
    textos = []
    nombre_automatizacion = "inventario_ajuste3_pdf"

    try:
        abrir_menu_lateral(driver, wait, nombre_automatizacion)
        mostrar_mensaje(driver, "Se realiza el ajuste de inventario 3", nombre_automatizacion)
        tomar_captura(driver, "captura_inicio_ajuste_inventario", "Se inició el proceso de inventario ajuste 3 correctamente.", capturas, textos, nombre_automatizacion)
        ...
        capturar_alerta_final(driver, wait, capturas, textos, nombre_automatizacion, timeout=12)
        ...
        print(f"Detalles del ajuste: Referencia producto: {ajuste_inventario_ajuste3_info.get('referencia_producto', 'No capturado')}, "
              f"Tipo transacción: {ajuste_inventario_ajuste3_info.get('tipo_transaccion', 'No capturado')}, "
              f"Causal transacción: {ajuste_inventario_ajuste3_info.get('causal_transaccion', 'No capturado')}, "
              f"fracciones: {ajuste_inventario_ajuste3_info.get('cantidad_fracciones', 'No capturado')}")
        ...
        escribir_log(nombre_automatizacion, f"Error durante el proceso de ajuste de inventario 3: {e}")
