"""
Script para automatizar el ingreso de mercancia a bodega 2
"""
import time
import os
import logging
...
def resaltar_elemento(driver, elemento, color="green", grosor="3px", duracion_ms=2000):
    try:
        ...
        escribir_log("ingreso_mercancia2_pdf", f"Error al resaltar elemento: {e}")
        raise
...
def buscar_producto(driver, wait, capturas, textos, nombre_automatizacion, codigo_producto="12957"):
    try:
        ...
        ingreso_mercancia2_info["codigo_producto"] = codigo_producto
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def seleccionar_tipo_transaccion(driver, wait, capturas, textos, nombre_automatizacion, tipo_transaccion_index=1):
    try:
        ...
        ingreso_mercancia2_info["tipo_transaccion"] = texto_opcion
...
def seleccionar_tipo_causal(driver, wait, capturas, textos, nombre_automatizacion, tipo_causal_index=2):
    try:
        ...
        # ✅ Guardar el texto en el diccionario
        ingreso_mercancia2_info["tipo_causal"] = texto_opcion
...
def ingresar_unidades(driver, wait, capturas, textos, nombre_automatizacion, unidades="30"):
    try:
        ...
        ingreso_mercancia2_info["unidades"] = unidades
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def ingreso_mercancia2(driver, codigo_producto="12957", tipo_transaccion_index=1, tipo_causal_index=2, unidades="30"):
    """
    ...
    capturas = []
    textos = []
    nombre_automatizacion = "ingreso_mercancia2_pdf"

    try:
        # abrir_menu_lateral(driver, wait, capturas, textos, nombre_automatizacion)
        mostrar_mensaje(driver, "Se realiza el ingreso de mercancía a bodega 2", nombre_automatizacion)
        ...
        print(f"Detalles del ingreso de mercancía: Código producto: {ingreso_mercancia2_info.get('codigo_producto', 'No capturado')}, "
              f"Tipo de transacción: {ingreso_mercancia2_info.get('tipo_transaccion', 'No capturado')}, "
              f"Tipo de causal: {ingreso_mercancia2_info.get('tipo_causal', 'No capturado')}, "
              f"Unidades: {ingreso_mercancia2_info.get('unidades', 'No capturado')}")
