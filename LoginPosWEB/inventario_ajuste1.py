"""
Script para automatizar el ajuste de inventario A- 1.
"""
import time
import os
import logging
...
ajuste_inventario_ajuste1_info = {}
...
def ingresar_referencia_producto(driver, wait, capturas, textos, nombre_automatizacion, referencia="12957"):
    try:
        ...
        ajuste_inventario_ajuste1_info["referencia_producto"] = referencia
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def seleccionar_tipo_transaccion(driver, wait, capturas, textos, nombre_automatizacion, tipo_transaccion="A-"):
    try:
        ...
        ajuste_inventario_ajuste1_info["tipo_transaccion"] = tipo_transaccion
...
def seleccionar_causal_transaccion(driver, wait, capturas, textos, nombre_automatizacion, causal_index=1):
    try:
        ...
        if causal_index < len(opciones):
            ...
            ajuste_inventario_ajuste1_info["causal_transaccion"] = f"Índice {causal_index}"
            time.sleep(2)  # Esperar antes de continuar con la siguiente acción
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def ingresar_cantidad_unidades(driver, wait, capturas, textos, nombre_automatizacion, unidades="1"):
    try:
        ...
        ajuste_inventario_ajuste1_info["cantidad_unidades"] = unidades
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def inventario_ajuste1(driver, referencia="12957", tipo_transaccion="A-", causal_index=1, unidades="1"):
    """
    Automatiza el proceso de ajuste de inventario 1 en el sistema, con logs y capturas.
    """
    wait = WebDriverWait(driver, 40)
    capturas = []
    textos = []
    nombre_automatizacion = "inventario_ajuste1_pdf"

    try:
        abrir_menu_lateral(driver, wait, nombre_automatizacion)
        mostrar_mensaje(driver, "Se realiza el ajuste de inventario 1", nombre_automatizacion)
        tomar_captura(driver, "captura_inicio_ajuste_inventario", "Se inició el proceso de inventario ajuste 1 correctamente.", capturas, textos, nombre_automatizacion)
        ...
        print(f"Detalles del ajuste: Referencia producto: {ajuste_inventario_ajuste1_info.get('referencia_producto', 'No capturado')}, "
              f"Tipo transacción: {ajuste_inventario_ajuste1_info.get('tipo_transaccion', 'No capturado')}, "
              f"Causal transacción: {ajuste_inventario_ajuste1_info.get('causal_transaccion', 'No capturado')}, "
              f"Unidades: {ajuste_inventario_ajuste1_info.get('cantidad_unidades', 'No capturado')}")
        ...
        escribir_log(nombre_automatizacion, f"Error durante el proceso de ajuste de inventario 1: {e}")
