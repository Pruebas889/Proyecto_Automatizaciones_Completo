"""
Script para automatizar el ajuste de inventario 1.
"""
import time
import os
import logging
...
def ingresar_referencia_producto(driver, wait, capturas, textos, nombre_automatizacion, referencia="12957"):
    try:
        ...
        ajuste_inventario1_info["referencia_producto"] = referencia
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def seleccionar_tipo_transaccion(driver, wait, capturas, textos, nombre_automatizacion, tipo_transaccion="A+"):
    try:
        ...
        ajuste_inventario1_info["tipo_transaccion"] = tipo_transaccion
...
def seleccionar_causal_transaccion(driver, wait, capturas, textos, nombre_automatizacion, causal_index=1):
    try:
        ...
        if causal_index < len(opciones):
            ...
            ajuste_inventario1_info["causal_transaccion"] = f"Índice {causal_index}"
            time.sleep(2)  # Esperar antes de continuar con la siguiente acción
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def ingresar_cantidad_unidades(driver, wait, capturas, textos, nombre_automatizacion, unidades="1"):
    try:
        ...
        ajuste_inventario1_info["cantidad_unidades"] = unidades
        time.sleep(0.5)
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def inventario1(driver, referencia="12957", tipo_transaccion="A+", causal_index=1, unidades="1"):
    """
    ...
    capturas = []
    textos = []
    nombre_automatizacion = "ajuste_inventario1_pdf"
    ...
    try:
        ...
        tomar_captura(driver, "captura_inicio_ajuste_inventario", "Se inició el proceso de ajuste de inventario 1 correctamente.", capturas, textos, nombre_automatizacion)
        ...
        # Capturar la alerta final que confirma 'Ajustes realizados correctamente.'
        ...
        print(f"Detalles del ajuste: Referencia producto: {ajuste_inventario1_info.get('referencia_producto', 'No capturado')}, "
              f"Tipo transacción: {ajuste_inventario1_info.get('tipo_transaccion', 'No capturado')}, "
              f"Causal transacción: {ajuste_inventario1_info.get('causal_transaccion', 'No capturado')}, "
              f"Unidades: {ajuste_inventario1_info.get('cantidad_unidades', 'No capturado')}")
