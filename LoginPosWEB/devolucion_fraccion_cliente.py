"""
Script para automatizar la devolución de una compra de fracciones con cliente, incluyendo logs y guardando los datos en una variable.
"""
import time
import os
import logging
...
from venta_fraccion_cliente import venta_fracciones_cliente_info
...
def ingresar_numero_factura(driver, wait, capturas, textos, nombre_automatizacion):
    try:
        factura = venta_fracciones_cliente_info.get("factura")
        if not factura:
...
def ingresar_numero_caja(driver, wait, capturas, textos, nombre_automatizacion):
    ...
    try:
        ...
        venta_fracciones_cliente_info["numero_caja"] = numero_caja_asignada
...
def seleccionar_causal_devolucion(driver, wait, capturas, textos, nombre_automatizacion, indice_causal=3):
    try:
        ...
        esperar_carga_desaparezca(driver, nombre_automatizacion, "seleccionar causal de devolución")
        ...
        venta_fracciones_cliente_info["causal"] = causal_texto
...
def confirmar_devolucion_final(driver, wait, capturas, textos, nombre_automatizacion):
    try:
        ...
        venta_fracciones_cliente_info["estado"] = "completado"
...
def fraccion_devolucion_cliente(driver):
    """
    ...
    try:
        ...
        mostrar_mensaje(driver, "Se efectuará la devolución de la compra de fracciones", nombre_automatizacion)
        ...
        print(f"Devolución completada para factura: {factura}, Caja: {venta_fracciones_cliente_info.get('numero_caja', 'No capturado')}, Causal: {venta_fracciones_cliente_info.get('causal', 'No capturada')}")
