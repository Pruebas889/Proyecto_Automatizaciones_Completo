"""
Script para automatizar la salida de mercancia a bodega 6
"""
import time
import os
import logging
...
def resaltar_elemento(driver, elemento, color="green", grosor="3px", duracion_ms=2000):
    try:
        ...
        escribir_log("salida_mercancia6_pdf", f"Error al resaltar elemento: {e}")
        raise
...
def seleccionar_tipo_transaccion(driver, wait, capturas, textos, nombre_automatizacion, tipo_transaccion_index=2):
    try:
        ...
        salida_mercancia6_info["tipo_transaccion"] = texto_opcion
...
def seleccionar_tipo_causal(driver, wait, capturas, textos, nombre_automatizacion, tipo_causal_index=1):
    try:
        ...
        # ✅ Save the text to the dictionary
        salida_mercancia6_info["tipo_causal"] = texto_opcion
...
def buscar_producto(driver, wait, capturas, textos, nombre_automatizacion, codigo_producto="12957"):
    try:
        ...
        salida_mercancia6_info["codigo_producto"] = codigo_producto
    except (TimeoutException, NoSuchElementException, StaleElementReferenceException) as e:
...
def salida_mercancia6(driver, codigo_producto="12957", tipo_transaccion_index=2, tipo_causal_index=1):
    """
    ...
    capturas = []
    textos = []
    nombre_automatizacion = "salida_mercancia6_pdf"

    try:
        # abrir_menu_lateral(driver, wait, nombre_automatizacion)
        mostrar_mensaje(driver, "Se realiza la salida de mercancía a bodega 6", nombre_automatizacion)
        ...
        print(f"Detalles de la salida de mercancía: Código producto: {salida_mercancia6_info.get('codigo_producto', 'No capturado')}, "
              f"Tipo de transacción: {salida_mercancia6_info.get('tipo_transaccion', 'No capturado')}, "
              f"Tipo de causal: {salida_mercancia6_info.get('tipo_causal', 'No capturado')}")
